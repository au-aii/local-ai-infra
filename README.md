...
3. **ネットワーク**: マルチデバイスアクセスに **Tailscale** を前提とする。Open WebUI は全インタフェース (`0.0.0.0:3000`) にバインドされているが、Tailscale の WireGuard 暗号化により Tailnet 外からはアクセスできない。Tailscale を使わない場合はポートバインドを `127.0.0.1:3000:8080` に変更し SSH ポートフォワードで運用すること。
...
| アクセス元 | URL | 前提 |
|------------|-----|------|
| WSL 内 / Windows | `http://localhost:3000` | そのまま |
| Mac / スマホ | `http://<WSL の Tailscale IP>:3000` | Tailscale 必須 |
| Mac (Tailscale なし) | `http://localhost:3000` | SSH ポートフォワード (後述) |

Open WebUI は `0.0.0.0:3000` にバインドされているが、Tailscale の WireGuard により Tailnet 外からはアクセスできない。Tailscale トラフィックは暗号化されるため HTTP でも安全である。
